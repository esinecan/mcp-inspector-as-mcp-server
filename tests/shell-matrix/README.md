...
| F5 | pointer aliases | `--within /record/body`, `record/body` and `#/record/body` all return the outline; under Git Bash the first form is rewritten by MSYS, so the other two must work there |
